...
- **Demo account seed is implemented locally:** `scripts/demo_seed/` contains a dry-run-first Node/Firebase Admin SDK seeder that writes 50 deterministic, synthetic documents across the existing personal-history collections only after `--apply --confirm-demo`. It resolves the account by registered email or UID, never deletes data or writes the profile, and keeps service-account JSON outside the repo. Pure script tests pass 4/4; no live Firebase write has been performed.
