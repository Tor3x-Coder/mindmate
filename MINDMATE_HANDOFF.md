...
- **Floating Tide Orb navigation is implemented and polished:** four lower-positioned destinations, slower 520ms-base glide, restrained hop, visible labels, semantics, reduced-motion support, and existing IndexedStack state preservation. Local visual validation is pending.
- **First-use guide is implemented:** four one-time coach marks for Home, Practice, Chat, and Me with Skip/Next/Got it, `tourVersion = 1` persistence, Replay app tour in Settings, and a lightweight 2D figure drawn with Flutter. Automatic display is requested only after new-user onboarding; Login/Splash do not force it. It never appears on Emergency Support and never autoplays speech.
- **Quiet Tide Modern shell is focused, not a full redesign:** global app bars now use consistent height/spacing and no scroll tint/elevation. Existing top-level content logic remains intact; child-screen polish is deferred until this shell passes.
...
Pull the Quiet Tide Modern shell batch. Run local analyzer/tests/build, then fully restart Chrome. Verify the slower/lower four-tab bar, automatic tour after a fresh onboarding path, Skip/Next/Got it, Settings → Replay app tour, tab-state preservation, and the 8-cue Quick Reset timeline. Do not expand narration, add ambience, or redesign child screens until this shell passes.
