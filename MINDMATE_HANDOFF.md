...
- **Weekly Progress insight is implemented locally; validation is next:** `ProgressScreen` computes a read-only last-seven-days `Your week` card from existing mood, journal, wellness, and feedback records. It shows activity counts, uses cautious language for a heavier day, and only mentions practices when feedback supports that observation. No Firestore writes, schema changes, AI calls, or clinical score were added. Run the focused weekly test, analyzer, and seeded-demo UI check before starting the daily reminder.
