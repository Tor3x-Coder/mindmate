...
| Mode-aware AI Worker and safety route | Batch 10 hardened; Learn article context implemented locally | **13/13 Worker tests passed; developer PC full Flutter suite 47/47; analyzer 0 errors/0 warnings** | **Existing `2026-08-23-batch10` Worker remains live; deployment of `2026-09-02-learn-context` approved but pending authenticated Cloudflare dashboard/CLI access** | Existing health/plan/calm/crisis routes remain verified; Learn-context live verification is pending the approved deployment and POST smoke matrix |
...
| Learn section | Home featured card, four category shelves, sixteen core reads, eight local Explore more reads, Add to Learn persistence, and article-scoped AI context implemented | Developer PC: `flutter analyze` **0 errors/0 warnings**; focused Learn test **2/2**, catalogue/Chat tests **6/6**, full Flutter suite **47/47** | App code not deployed; Worker source is versioned `2026-09-02-learn-context`; deployment is approved but live Worker remains Batch 10 pending authenticated access and binding verification | Chrome/device Learn flow, Add to Learn persistence, live article-context chat, Android/Web build, and health-literate review of expanded/urgent content remain open |
...
The article-scoped AI path sends only the selected approved article as bounded reference context to the existing Chat screen and Worker. Chat shows the selected article, lets the user clear the context, and keeps the existing crisis-first and AI-boundary behavior. The new Worker source is versioned `2026-09-02-learn-context`; deployment is approved but still pending authenticated Cloudflare access and verification of the existing `AI`, model, rate-limit, and metrics bindings. The currently live Worker remains the previously verified Batch 10 version until that deployment and smoke check complete.

Current Learn validation state: implementation and automated validation are now complete for this batch. On the developer PC, `flutter analyze` reports 0 errors/0 warnings; the focused Learn widget test passes 2/2, the catalogue/Chat test command passes 6/6, and the full Flutter suite passes 47/47. The developer's broad `dart format lib test` changes remain safely in the local stash named `local formatting before Learn test fix`. Android/Web builds, Chrome/device checks, and the health-literate skim of articles 3–4 remain later release validation. Worker deployment is approved, but authenticated Cloudflare access, binding verification, deployment, and live context smoke remain open. Nothing from the app expansion is deployed yet.
