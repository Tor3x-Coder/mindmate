...
| Mode-aware AI Worker and safety route | Batch 10 hardened | **12/12 Worker tests passed; 4 ChatService tests passed (9/9 total)** | **Deployed — `mindmate-ai-chat`** | **Verified live `/health` (2026-08-23-batch10), plan, calm, and crisis route** |
...
| Learn section | Home featured card, topic list, reader, and six bundled static articles implemented | Developer PC: `flutter analyze` passed with 0 errors/0 warnings and 23 informational notices after the const fix; first `flutter test` run reached 40/41, and a second run reached 40/41 because the widget test tapped the first card after scrolling to the lazily-built final card; test now keeps the visible first-card navigation check and the content test covers all six articles, rerun pending | N/A — no backend or deployment change | Chrome/device layout, article navigation, and health-literate skim of articles 3–4 remain open |

### Learn section — implemented, validation pending

The approved Learn Option B is now in the app code. Home places a featured Learn card directly between the Wellness card and Quick starts. The card opens a topic list with title, one-line description, and read-time hint. Each topic opens a scrollable article reader with a calm header, readable sections, a general-information disclaimer, and one real next-step button into an existing MindMate tool.

The six offline articles cover: things that quietly support your mind; things that quietly damage your mind; substances and your brain (including cannabis, alcohol, codeine-based syrups, tramadol, and inhalants with myth/reality framing); when coping becomes a problem; getting help in Nigeria; and if your friend is struggling. No Firestore collection, network request, or new personal data is used.

Current Learn validation state: implementation is present. The developer-PC analyzer now reports 0 errors/0 warnings and 23 informational notices after the unused-const Learn fix. The first test run reached 40/41 because the widget test checked the final topic before a lazily-built `ListView` had built it. The second run reached 40/41 because the test then tried to tap the first card after scrolling away from it. The test now keeps the reader-navigation tap on the visible first card while the static content test covers all six articles; rerun is pending. Android/Web builds, Chrome/device checks, and the health-literate skim of articles 3–4 remain open. Nothing is deployed from this batch.
...
| 2 Sep 2026 | Learn widget-test visibility fixes | Made the intro icon const and kept the widget navigation tap on the visible first card instead of assuming all lazy list cards are mounted; static content test still covers all six articles | Developer PC analyzer: 0 errors/0 warnings/23 info; two test runs reached 40/41 because of lazy-list test ordering; patched, rerun pending | Not deployed; no backend or data change | Re-run `flutter test`, then build and run the Learn flow in Chrome/device; review articles 3–4 |
