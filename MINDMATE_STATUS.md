...
| Mode-aware AI Worker and safety route | Batch 10 hardened; Learn article context implemented locally | **13/13 Worker tests passed; Flutter client tests pending on developer PC** | **Existing `2026-08-23-batch10` Worker remains live; new context source intentionally not deployed in this code-only batch** | Existing health/plan/calm/crisis routes verified; Learn-context live verification is deferred until a Worker deployment is separately approved |
...
| Learn section | Home featured card, four category shelves, sixteen core reads, eight local Explore more reads, Add to Learn persistence, and article-scoped AI context implemented | Developer PC: `flutter analyze` **0 errors/0 warnings, 29 informational notices**; focused Learn test **2/2**, catalogue/Chat tests **6/6**, full Flutter suite **43/43** after the viewport fix | App code not deployed; Worker source is versioned `2026-09-02-learn-context` but live Worker remains Batch 10 until a separately approved deployment | Chrome/device Learn flow, Add to Learn persistence, live article-context chat, Android/Web build, and health-literate review of expanded/urgent content remain open |
| Demo account history seed | One-off Node/Firebase Admin SDK script implemented with deterministic IDs, explicit dry-run default, and no-delete boundary | Script tests **4/4**; syntax check and 50-document dry run passed; live Firebase write not run | Not deployed; no account data written from Arena | Register the demo account first, then run the documented `--apply --confirm-demo` command only against that account |
...
The article-scoped AI path sends only the selected approved article as bounded reference context to the existing Chat screen and Worker. Chat shows the selected article, lets the user clear the context, and keeps the existing crisis-first and AI-boundary behavior. The new Worker source is versioned `2026-09-02-learn-context`; its Cloudflare deployment is intentionally deferred because deployment was not part of this code-only batch. The currently live Worker remains the previously verified Batch 10 version, so article-context behavior will become live only after a separately approved deployment and smoke check.

Current Learn validation state: implementation and automated validation are now complete for this batch. On the developer PC, `flutter analyze` reports 0 errors/0 warnings and 29 informational notices; the focused Learn widget test passes 2/2, the catalogue/Chat test command passes 6/6, and the full Flutter suite passes 43/43. The developer's broad `dart format lib test` changes remain safely in the local stash named `local formatting before Learn test fix`. Android/Web builds, Chrome/device checks, and the health-literate skim of articles 3–4 remain later release validation. Worker deployment/live context smoke is a later, separately approved action. Nothing from this expansion is deployed yet.

### Demo account history seed — implemented locally, live write intentionally pending
...
| 2 Sep 2026 | Learn expansion + article-scoped AI context | Added four Learn shelves, ten core scenario reads, eight local Explore more reads with search/Add to Learn persistence, conversational headings, Ask MindMate article context, bounded ChatService/Worker payload support, and tests | Arena static checks passed; Worker `npm test` **13/13** and `node --check` passed; Flutter PC rerun is pending after the expansion | Flutter app not deployed; Worker deployment intentionally deferred (live Worker remains Batch 10) | Run PC `flutter analyze`/`flutter test`, Android/Web builds, and review urgent articles and articles 3–4; deploy the Worker only if separately approved |
| 2 Sep 2026 | Learn Chat widget-test visibility patch | The article test now jumps the reader scroll position to its end and taps the actual `OutlinedButton`, so the Ask MindMate action must be inside the test viewport; no app behavior changed | Developer PC: analyzer **0 errors/0 warnings, 29 informational notices**; focused Learn **2/2**, catalogue/Chat **6/6**, and full Flutter suite **43/43** passed after the fix | Flutter app not deployed; Worker deployment intentionally deferred | Continue the documented validation order: health-literate review and later Chrome/device/release checks; no build or Worker deployment in this step |
| 2 Sep 2026 | Demo account seed script | Added a one-off Firebase Admin SDK script with repeatable 21-day synthetic history, 50 production-shaped documents, target-account guard, dry-run default, explicit apply confirmation, and credential safety documentation | Node seed tests **4/4**; `node --check` passed; 50-document dry run passed; no live Firebase write performed | Not deployed; no account data written from Arena | Register the demo account through the app, then apply the script only when the team is ready; do not commit or share the service-account JSON |
