...
| Weekly Progress insight | Read-only seven-day summary implemented from existing moods, journals, wellness reflections, and feedback | Pure `WeeklyInsight` tests pending on the developer PC; no new storage or AI call | N/A | Demo-app UI check pending; then continue to the daily reminder |
...
### Weekly Progress insight — implemented locally, validation pending
...
The card reads existing feedback through a UID-only Firestore query and sorts that small list on the client to avoid introducing a composite index. Pure `WeeklyInsight` tests are present; the developer should run them with the normal Flutter test suite and check the seeded demo account's Progress screen before moving to the local reminder.
...
| 2 Sep 2026 | Weekly Progress insight | Added a read-only seven-day summary card with counts, a cautious heavier-day observation, practice feedback context, empty state, and UID-only feedback loading | Pure `WeeklyInsight` test command pending on the developer PC; no new storage or AI call | Not a deployment; no new backend data | Pull this batch, run the focused weekly test and full Flutter suite, then inspect the demo Progress screen before starting the local reminder |
