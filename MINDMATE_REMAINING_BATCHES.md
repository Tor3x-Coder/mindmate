...
2. **Demo account via Admin SDK script — implemented and applied 2 September 2026.** `scripts/demo_seed/seed_demo_data.mjs` targets a registered account by email or UID and defaults to a credential-free dry run. With explicit `--apply --confirm-demo`, it writes 50 deterministic documents across the existing production-shaped `mood_logs`, `journal_entries`, `wellness_assessments`, `meditation_history`, `breathing_sessions`, `thought_records`, and `feedback_records` collections. It never writes the user profile, deletes data, or commits credentials. Script tests pass 4/4; the developer successfully applied the seed to the dedicated demo account. Confirm the seeded counts in the app; the account can be reseeded safely if the demo data needs refreshing.
